
var touchup = require('./');

var sourceURL = 'data:image/png;base64,';

var imageElement = document.createElement('img');
imageElement.addEventListener('load', function() {
  document.body.appendChild(document.createTextNode('source:'));
  document.body.appendChild(imageElement);

  var repeatedURL = touchup.repeat(imageElement, 2, 2);
  var repeatedImageElement = document.createElement('img');
  repeatedImageElement.addEventListener('load', function() {
    document.body.appendChild(document.createTextNode('repeated:'));
    document.body.appendChild(repeatedImageElement);
  });
  repeatedImageElement.setAttribute('src', repeatedURL);
});
imageElement.setAttribute('src', sourceURL);


